# sentiment_fusion_manager.py - 개선된 버전
...
# 가정: sentiment_analyzer는 이미 개선된 버전
...
class SentimentFusionManager:
    ...
    def get_fused_scores(self, 
        ...
        for article in articles:
            try:
                # 개별 분석
                detail = self.analyzer.analyze_detailed(article)
                
                # 단일 소스로 융합 (확장 가능)
                fused_score = self.fuse(
                ...
                # 결과 구성
                result = {
                    "datetime": article.get("datetime", datetime.utcnow()),
                    "symbol": symbol,
    ...
    def save_state(self, filepath: Union[str, Path]):
        ...
        state = {
            'config': {
                ...
                'correction_factor': self.config.correction_factor,
            ...
            'history': [
                ...
                for h in self.history[-100:]  # 최근 100개만 저장
            ],
    ...
    def load_state(self, filepath: Union[str, Path]):
        ...
        try:
            ...
            self.config = FusionConfig(**config_data)
            
            # 적응형 가중치 복원
            self.adaptive_weights = state.get('adaptive_weights', {})
            self.weight_update_count = state.get('weight_update_count', 0)
            
            # 히스토리 복원
            self.history.clear()
            for h_data in state.get('history', []):
                self.history.append(FusedSentiment(
                    timestamp=datetime.fromisoformat(h_data['timestamp']),
                    symbol=h_data['symbol'],
                    fused_score=h_data['fused_score'],
                    raw_scores=h_data['raw_scores'],
                    weights_used=h_data['weights_used'],
                    confidence=h_data['confidence'],
                    volatility=h_data['volatility'],
                    trend=h_data['trend'],
            ...
            self.performance_metrics = defaultdict(
                lambda: {'accuracy': [], 'confidence': [], 'volatility': []},
...
if __name__ == "__main__":
    ...
    score1 = fusion_manager.fuse({"news": 0.7})
    ...
    score2 = fusion_manager.fuse(multi_scores)
    ...
    score3 = fusion_manager.fuse(outlier_scores)
    ...
    for i in range(10):
        ...
        fused = fusion_manager.fuse(scores, symbol=symbol)
    ...
    test_articles = [
        {
            ...
            "snippet": "Cryptocurrency market shows strong bullish momentum",
            "datetime": datetime.utcnow()
        },
        {
            "title": "Market Crash Fears Grow Amid Uncertainty",
            "snippet": "Investors flee to safety as volatility spikes",
            "datetime": datetime.utcnow()
        }
    ]
    
    results = fusion_manager.get_fused_scores(test_articles)
    ...
    # 저장
    state_file = Path("test_fusion_state.json")
    fusion_manager.save_state(state_file)
    print(f"   State saved to {state_file}")
    
    # 새 인스턴스 생성 및 로드
    new_manager = SentimentFusionManager()
    new_manager.load_state(state_file)
    print(f"   State loaded successfully")
    print(f"   History size: {len(new_manager.history)}")
